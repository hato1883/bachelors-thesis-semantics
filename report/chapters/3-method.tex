...
\subsection*{Syntactic Categories and Domains}
...
Using the keyword \texttt{inductive} starts the definition, where we then give it a name, in this case \emph{Num}, we then continue with the keyword \emph{where}
which marks the start of the case by case definition.
Next we give our inductive case a name, then follow it by its type, in this case \emph{Num}.
On the last two lines we also include a variable $n$, this marks a recursive relationship to itself.

\paragraph{Comparison and Result: Numerals}
The discretization of numerals in Lean 4 requires a constructive approach compared to the meta-level assumption of numbers in the textbook.
While the textbook allows for binary representation at a syntactic level, the Lean version treats them as a distinct inductive type, mirroring the structure but enforcing type safety.
...
\paragraph{Comparison and Result: Arithmetic Expressions}
The mapping of arithmetic expressions is nearly one-to-one. The primary difference lies in the explicit naming of constructors (e.g., \texttt{add}, \texttt{mul}) vs the infix notation used in pen-and-paper. In Lean, we latter define notation to restore this readability, but the underlying structure remains a tree of constructors.
...
\end{paracol}
\vspace{1em}
One interesting decision is to use the mathematical \texttt{true} and \texttt{false} instead of the \textbf{tt} and \textbf{ff} often used in semantic literature.
This decision was made to be able to use the built-in methods in Lean to reason about boolean algebra, such as negation of a negation etc.

\paragraph{Comparison and Result: Boolean Expressions}
Utilizing Lean's built-in \texttt{Bool} capabilities through "Boolean Realism" allows for the reuse of existing boolean logic theorems. This significantly reduces the overhead needed to prove properties like double negation, which in the textbook would require manual development of the truth-value semantics.

Lastly we define our statements:
\begin{paracol}{2}
    \begin{leftcolumn}
        \begin{Lean_Code}
...
\paragraph{Comparison and Result: Statements}
The formalization of the \textit{While} statements illustrates the transition from mathematical notation to a typed system:
\begin{itemize}
    \item \textbf{Namespace Management}: Lean 4 uses keywords like \texttt{if} and \texttt{while} for its own control flow. To avoid collisions, the mechanized version uses \texttt{cond} (for \texttt{if}) and \texttt{loop} (for \texttt{while}). This highlights a practical constraint of embedding a language into a host language.
    \item \textbf{Typed BNF}: While the textbook uses symbolic BNF, Lean requires explicit typing for constructors (e.g., \texttt{composition (S1 S2 : Stmt)}). This provides immediate feedback on the "well-formedness" of statements.
\end{itemize}

\subsection*{Semantic Domains and State}
...
\paragraph{Comparison and Result: Semantic Functions}
Formalizing the evaluation functions (\(\mathcal{N}, \mathcal{A}, \mathcal{B}\)) reveals the strictness of the mechanized environment:
...
However, we can provide a proof by extension:
\begin{quotation}
    Two states $s_1$ and $s_2$ are equal if for all variables $y$, $s_1 y = s_2 y$.
    Let $s' = s[x \mapsto z_1][x \mapsto z_2]$.
    By definition of assignment:
    \[
    s' y = \begin{cases} 
        z_2 & \text{if } y = x \\ 
        s[x \mapsto z_1] y & \text{if } y \neq x 
        \end{cases}
    \]
    If $y = x$, then $s' x = z_2$. This matches $(s[x \mapsto z_2])x = z_2$.
    If $y \neq x$, then $s' y = s[x \mapsto z_1] y$. Again by definition of assignment, since $y \neq x$, $s[x \mapsto z_1] y = s y$.
    This matches $(s[x \mapsto z_2])y = s y$ for $y \neq x$.
    Thus, for all $y$, $s' y = (s[x \mapsto z_2]) y$, so $s' = s[x \mapsto z_2]$.
\end{quotation}
...
However, we can provide a proof by extension:
\begin{quotation}
    Let $s_1 = s[x \mapsto z_1][y \mapsto z_2]$ and $s_2 = s[y \mapsto z_2][x \mapsto z_1]$. Assume $x \neq y$.
    For any variable $v$:
    \begin{itemize}
        \item If $v = y$, then $s_1 v = z_2$ by definition of the outer assignment. For $s_2$, since $y \neq x$ and $v = y$, $s_2 y = s[y \mapsto z_2] y = z_2$.
        \item If $v = x$, then $s_2 v = z_1$ by definition of the outer assignment. For $s_1$, since $x \neq y$ and $v = x$, $s_1 x = s[x \mapsto z_1] x = z_1$.
        \item If $v \neq x$ and $v \neq y$, then $s_1 v = s[x \mapsto z_1] v = s v$ and $s_2 v = s[y \mapsto z_2] v = s v$.
    \end{itemize}
    In all cases $s_1 v = s_2 v$, hence $s_1 = s_2$.
\end{quotation}
...
\]
which otherwise might not be obvious for the reader, and definitely not for the Lean 4 compiler.
