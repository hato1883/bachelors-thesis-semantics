...
We procced by cases on how the first step of \(\langle S, s \rangle \Rightarrow^{k_+1} s'\)
...
if one is deﬁned on a state s, then so is the other, and therefore if one is not
deﬁned on a state s, then neither is the other.
\end{quotation}
\end{rightcolumn}
\end{paracol}
\vspace{1em}
