...
\begin{itemize}
    ...
    The literature defines numerals as binary numbers via inductive definition;
...
via exhaustive pattern matching, whereas textbooks define these as inductive relations
...
mechanization works with evaluation relations. Lean can represent partial functions via dependent types,
...
\begin{itemize}
    ...
    \item \textbf{Generalization Strategy}: Variables that should not be fixed during induction must be explicitly generalized (\autoref{discussion:sec:multistep}), a concern invisible in informal mathematics.
...
Partial functions cannot be directly formalized in dependent type theory; relations
