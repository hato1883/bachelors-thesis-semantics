...
The transition system for Natural Semantics is formalized as an inductive relation
\texttt{big\_step} that captures the complete execution of a \While program from
initial state to final state. This big-step approach mirrors the pen-and-paper
formulation: each rule represents either an axiom (for atomic statements like
assignment and skip) or a deductive rule (for composite constructs).
The relation mechanizes the textbook's informal rules \missingcitation \todo{Cite the specific textbook these rules are mirrored from} by encoding:
\begin{itemize}
...
By splitting the equivalence (\(\iff\)) into two implications (\(\implies\) and
\(\impliedby\)), we can prove each direction independently.
This modularity is mirrored in Lean through separate theorems
(\texttt{while\_expand} and \texttt{if\_compact}) which are then combined using
\texttt{Iff.intro}. Key observations include:

\begin{itemize}
    \item \textbf{Syntactic Correspondence}: The mechanized proof maps nearly 1:1
    to the textbook's structure. Each "case" in Lean corresponds to a branch
    in the manual proof, and each rule application (e.g., \texttt{big\_step.comp})
    matches the "construction of a new tree" in the text.
    \item \textbf{Explicit vs. Implicit Construction}: In Lean, we use the \texttt{have}
    tactic to explicitly name intermediate derivation trees (like \texttt{comp\_stmt}).
    The textbook does this narratively ("we can construct the derivation tree...").
    \item \textbf{Exhaustive Matching}: Lean's \texttt{cases} tactic on the \texttt{if}
    statement automatically identifies that only \texttt{if\_true} and \texttt{if\_false}
    are possible, mirroring the textbook's "Only two rules could give rise to...".
\end{itemize}
...
\begin{itemize}
    \item \textbf{Atomic Rules}: Assignment and skip both reduce to final
    configurations in a single step.

    \item \textbf{Decomposition Rules}: Composition rules split the execution:
    the first rule continues stepping the left component, and the second rule
    activates the right component once the left completes.

    \item \textbf{Conditional Rules}: The two conditional branches are explicitly
    separated, with the condition determining which rule applies.

    \item \textbf{While Loop Rule}: Rather than recursive application, the while
    loop is unrolled into an equivalent conditional-composition structure, which
    then proceeds by small steps.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Formalization of the Relation}: The textbook uses a somewhat
    informal "where \(k = k_1 + k_2\)" clause. In Lean, this must be part of
    the explicit existential bundle (\(\land k = k_1 + k_2\)),
    requiring the use of \texttt{constructor} to split the logical conjunctions.

    \item \textbf{Generalization}: The \texttt{generalizing} keyword is used in
    the induction to ensure that the induction hypothesis remains applicable to
    different states and statements as the execution progresses.

    \item \textbf{Specialization}: Tactics like \texttt{specialize} are used to "fix"
    a generalized induction hypothesis to a specific derivation (\texttt{h\_tail}),
    allowing for direct application to the sub-problem.

    \item \textbf{Existential Unpacking}: The mechanized proof uses \texttt{let} with
    pattern matching (the angle brackets) to "unpack" the witnesses from the
    induction hypothesis, whereas the textbook simply states "this means there
    is a state \(s_0\)".

    \item \textbf{Automation}: The proof relies on automation tactics like
    \texttt{simp} and \texttt{linarith} (or \texttt{linarity}) to handle the
    arithmetic bookkeeping regarding the step counts, which is often dismissed
    as "trivial" or "obvious" in manual proofs.
\end{itemize}
