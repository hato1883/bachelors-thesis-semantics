...
As mentioned we will start with formulating our toy language \While as defined in the textbook~\citep{nielson_semantics_2007}.
...
(Pen-and-Paper) which is directly quoted from the textbook by~\cite{nielson_semantics_2007}
...
\begin{itemize}
    ...
    The \texttt{generalizing} keyword is a crucial pedagogical gap.
    ...
    Due to our definition of Config type we must uses the \texttt{cases} tactic
    to build our intermediate \(\gamma = \langle S', s' \rangle\).
    The textbook performs this move implicitly by
    immediately discussing [comp\(_{sos}^{1}\)] and [comp\(_{sos}^{2}\)].
\end{itemize}
...
\begin{leftcolumn}
    \begin{Lean_Code}
  | while_true h_cond h_body
      h_rest ih_body ih_loop =>
    rename_i b S s s' s''
    -- Exercise 2.21
    have h_after_ex21 :=
      exercise_2_21
...
\begin{itemize}
    ...
    This can be done by prepending a '§' character or by using the \texttt{rename\_i} tactic.
...
\begin{leftcolumn}
    \begin{Lean_Code}
        case comp1 S1 S1' S2
          s_next h_step_S1 =>
          -- Apply Lemma 2.19 to split the sequence
