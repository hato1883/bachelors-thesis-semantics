...
First steps before reasoning whether Natural Semantics and Structural Operational Semantics are equivalent is to create them.
To create them we also need a language to apply them to, in our case this will be \While \missingcitation \todo{Reference the source where this version of the While language is defined (e.g., Nielson & Nielson)}.
...
Due to Lean 4 \missingcitation \todo{Cite the Lean 4 programming language/theorem prover official paper or manual} being a programming language, terms must be defined before they are used, which occasionally forces a different order than traditional textbook presentations.

\subsection*{Syntactic Categories}

We begin by mechanizing the syntactic categories defined in chapter 1.2 of the literature \missingcitation \todo{Add the specific citation for the 'literature' mentioned here}. 
...
\begin{itemize}
    ...
    \item \textbf{Explicit Typing}: Each constructor explicitly specifies its type, providing immediate feedback on well-formedness, unlike the meta-level assumptions in pen-and-paper BNF \missingcitation \todo{Optional: cite a source discussing the differences between informal meta-theory and formal mechanization, e.g., 'Software Foundations' or similar}.
\end{itemize}
...
The \textbf{State} represents memory as a total function from variables to integers \missingcitation \todo{Cite the standard definition of State in operational semantics}:
\begin{paracol}{2}
...
In the textbook \missingcitation \todo{Cite the specific textbook where Lemma 2.5 is found (e.g., Nielson \& Nielson)} they provide a lemma stating the following:
\begin{quote}
The statement
...
\]
Making sure to label them the same way they do in the textbook \missingcitation \todo{Cite the source to justify the consistency of these labels (* and **)} we get the following:
\[
...
the second part proves the reverse, namely proving that given (**) that (*) holds. Which is the normal way to prove equivalence of two statements \missingcitation \todo{Optional: Cite a reference on basic proof techniques or logic if this is a pedagogical thesis}.

\subsubsection*{Forward Direction: $(*) \implies (**)$}
Let us now compare the textbook proof \missingcitation \todo{Reference the specific page or section in the textbook for this proof direction} with the mechanized equivalent for the case where we show that (*) holds, implies that (**) holds.
\begin{paracol}{2}
\begin{leftcolumn}
    \centering \textbf{Mechanized (Lean 4)}
\end{leftcolumn}
...
Now we will show that (**) holds, implies that (*) holds \missingcitation \todo{Reference the specific section in the textbook for the reverse proof direction}.
\begin{paracol}{2}
\begin{leftcolumn}
    \centering \textbf{Mechanized (Lean 4)}
\end{leftcolumn}
...
The next proof made in the textbook\missingcitation\todo{Cite the specific textbook being referred to here (presumably Nielson \& Nielson).} refers to determinism, where the definition of determinism is given
\begin{quotation}
...
The transition system for Structural Operational Semantics is formalized as an inductive relation \texttt{small\_step} that captures single-step program execution. A key innovation in the mechanized version is the introduction of the \texttt{Config} type... [Personal narrative - no citation needed here]
...
notation for these\missingcitation\todo{Cite textbook for $\Rightarrow^{k}$ and $\Rightarrow^{*}$ notations.} ($\langle S, s\rangle \Rightarrow^{k} s'$ for exactly
...
\begin{itemize}
    \item \textbf{Formalization of the Relation}: The textbook \missingcitation \todo{Reference the specific textbook being compared.} uses a somewhat informal "where \(k = k_1 + k_2\)" clause. In Lean, this must be part of the explicit existential bundle (\(\land k = k_1 + k_2\)), requiring the use of \texttt{constructor} to split the logical conjunctions.
    ...
    \item \textbf{Existential Unpacking}: The mechanized proof uses \texttt{let} with pattern matching (the angle brackets) to "unpack" the witnesses from the induction hypothesis, whereas the textbook \missingcitation \todo{Cite the source providing the pen-and-paper version of this step.} simply states "this means there is a state \(s_0\)".
...
\begin{quotation}
\noindent
For every statement S of \While, we have \(\mathcal{S}_{ns}[\![S]\!] = \mathcal{S}_{sos}[\![S]\!]\). \missingcitation \todo{This is a formal theorem statement quoted or adapted from a source; cite the source (e.g., Nielson and Nielson).}
...
The mechanized proof of \texttt{ns\_to\_sos} mirrors the textbook's inductive structure but requires explicit handling of Lean-specific machinery:
\begin{itemize}
    \item \textbf{Constructor-Based Induction}: The textbook performs induction on ``the shape of the derivation tree'' informally. Lean explicitly matches on constructors (\texttt{ass}, \texttt{skip}, \texttt{comp}, \texttt{if\_true}, \texttt{if\_false}, \texttt{while\_true}, \texttt{while\_false}) using \texttt{induction h with}.
    \item \textbf{Applying Lemmas Conditionally}: The textbook references Exercise 2.21 as a ``helper fact'' and applies it where needed. In Lean, this becomes an explicit call to \texttt{exercise\_2\_21} with the relevant statement \texttt{S2} specified as a named argument.
    \item \textbf{Transitivity}: The textbook's logical chaining (``we have already argued for the last part'') is formalized via \texttt{small\_step\_star\_trans}, which explicitly composes the multi-step derivations.
    \item \textbf{Variable Renaming}: The \texttt{rename\_i} tactic is used to introduce shadow variables introduced by induction (e.g., \(S_1, S_2, s, s', s''\)) into the local context when the inductive constructors do not bind them explicitly.
    \item \textbf{Decomposition vs. Composition}: While the textbook focuses on decomposing a complex case (composition) into simpler parts, the Lean proof must explicitly apply multiple inference rules (\texttt{small\_step.ass}, \texttt{small\_step.skip}, etc.) and use \texttt{apply small\_step\_star.step} to build up the derivation.
\end{itemize}
...
The mechanized proof of \texttt{sos\_k\_to\_ns} demonstrates how to formalize a proof by induction on the ``length'' of a derivation:
\begin{itemize}
    \item \textbf{Strong Induction}: The textbook states ``by induction on the length \(k\) of the derivation sequence''; Lean uses \texttt{Nat.strongRecOn} to enable strong induction, allowing the induction hypothesis to apply to any \(m < k_0\), not just \(m = k_0 - 1\). This is essential for reasoning about decomposed sub-derivations with unknown step counts.
    \item \textbf{Pattern Matching on Inductive Families}: The textbook separates the base case (\(k = 0\)) and inductive step (\(k = k_0 + 1\)) informally. In Lean, this is done with \texttt{match n with} to branch on the natural number, then \texttt{cases h\_k\_step} to decompose the derivation proof.
    \item \textbf{Composition Rule Handling}: The textbook treats [comp\(_{sos}^1\)] and [comp\(_{sos}^2\)] as two cases, and uses Lemma 2.19 to split the sequence. Lean calls \texttt{seq\_split} to obtain witnesses (\(s\_mid\), \(k_1\), \(k_2\)) and then applies the induction hypothesis to each piece. The two cases are \texttt{case comp1} and \texttt{case comp2}.
    \item \textbf{Justifying Step Counts}: The textbook glosses over arithmetic details like ``\(k_1 + k_2 = k_0 + 1\)'' or ``\(k_2 > 0\)''. In Lean, these must be explicitly verified: checking that \(k_2 > 0\) via \texttt{cases k₂}, and using \texttt{linarith} to discharge arithmetic goals.
    \item \textbf{Bridging Lemmas}: The while case uses a helper lemma \texttt{lemma\_2\_5} to convert between the natural semantics and structural operational semantics for while loops. This formalizes the informal statement ``from Lemma 2.5 we get the required''.
\end{itemize}
...
The proof of Theorem 2.26 (\texttt{ns\_equvivlent\_sos}) is concise in both the textbook and mechanized versions, but reveals important differences in how mechanized proofs handle logical equivalences:
\begin{itemize}
    \item \textbf{Biconditional Construction}: The textbook simply states ``\(\mathcal{S}_{ns}[\![S]\!] = \mathcal{S}_{sos}[\![S]\!]\)'' and argues both directions. Lean must explicitly construct a biconditional (\(\leftrightarrow\)) using \texttt{Iff.intro}, which takes the forward direction (\texttt{mp}, ``modus ponens'') and backward direction (\texttt{mpr}, ``modus ponens reverse'').
    \item \textbf{Delegating to Sub-Lemmas}: The proof of Theorem 2.26 in Lean does not repeat the inductive arguments; instead, it applies \texttt{ns\_to\_sos} for the forward direction and \texttt{sos\_to\_ns} for the backward direction. This mirrors the textbook's informal reference to Lemmas 2.27 and 2.28, but makes the logical dependencies explicit.
    \item \textbf{Reduction to Relations}: Both Lemma 2.27 (\texttt{ns\_to\_sos}) and Lemma 2.28 (\texttt{sos\_k\_to\_ns}) are formulated as implications on the evaluation relations (\(\to_{ns}\) and \(\to_{sos}\)) rather than on the semantic functions \(\mathcal{S}_{ns}\) and \(\mathcal{S}_{sos}\). This is a key difference: the textbook informal statement about ``functions'' is given a relational interpretation in Lean, which is more amenable to formal proof.
    \item \textbf{Semantic Function as Partial}: As noted in the TODO comments at the beginning of the section, the semantic function cannot be used directly in Lean 4 proofs (it is partial), so the proofs work with the underlying relations. This is a practical constraint of mechanized mathematics that the textbook omits.
\end{itemize}
