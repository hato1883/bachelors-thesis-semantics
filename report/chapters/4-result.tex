...
how an individual proof from the textbook compares with the corresponding Lean formulization.
...
being the explicit addition of the \texttt{DecidableEq} property to the Boolean type \textbf{T}.
As mentioned earlier this allows reasoning whether or not \(\mathtt{tt} == \mathtt{ff}\), as it stands we have not
defined that \(\mathtt{tt} == \mathtt{true}\) in Lean, something which will affect how we define the upcoming \texttt{Bexp\_eval}.
...
\begin{itemize}
    ...
    To use notation within the definition itself (recursive calls),
    \texttt{quotPrecheck} and \texttt{hygiene}
...
% \[
%     s[x \mapsto z_1][y \mapsto z_2] = s[y \mapsto z_2][x \mapsto z_1], \quad \mbox{when } y \neq x
% \]
...
\begin{itemize}
    ...
    annotations to justify the choice of a rule. Where the justification can be proven outside of the deduction tree,
...
\begin{itemize}
    ...
    In Lean, this conclusion is found automatically from the fact that there exist no sych number n,
    ...
    Unpacking by using the tactic \texttt{have} unpacks the deafult anonymous named components,
    allowing thier usage in the proof.
...
To follow the texbook we will make sure to prove them in the same order starting with Lemma 2.27,
but before we can prove Lemma 2.27 we need to introduce a auxiliary proof.
\begin{Lean_Code}
...
\begin{itemize}
    ...
    whilst Lean must use a auxilurary proof\\
...
something which lean dose automaticaly for us, allowing us to write shorter proofs, such in the case of
if true case.
...
if \(\langle S, s \rangle \Rightarrow^* s'\) then there must exist some \(k\) such that \(\langle S, s \rangle \Rightarrow^k s'\).
