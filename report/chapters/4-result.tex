...
In this chapter we present the result of the proposed method showing the mechanized proof in Lean next to the original definition given in the book.
The full Lean 4 implementation of the formalization and proofs is available in the public GitHub code repository \citep{bachelor_thesis_code_repo}.
We will begin by mechanizing the \While language definition into lean.
...
how the proofs from the textbook compares with the corresponding Lean formalizations.
...
We will start with formulating \While as defined in \emph{Semantics with Applications: An Appetizer}~\citep{nielson_semantics_2007}.
We will proceed in the same order as outlined by the book, starting with the syntactic categories,
before moving on to the semantic functions.
...
Below a side by side view is presented where the left side (Mechanized (Lean 4)) is formalizations of the text on the right
...
\begin{itemize}
    ...
    Crucially, to be able to compare two objects in Lean we need to enable it (e.g., to
...
\begin{rightcolumn}
    ...
    \begin{array}{rcl}
    \mathcal{B}[\![\neg b]\!]s &=& \begin{cases} 
        \textbf{tt} & \text{if } \mathcal{B}[\![b]\!]s = \textbf{ff} \\ 
...
The next section compares the formalization of Natural Semantics for the \While language,
followed by Structural Operational Semantics, and finally compares the equivalence proof.
...
When it comes to operational semantics we use derivation tree syntax to provide inference rules and axioms.
...
\begin{prooftree}
  ...
  \hypo{
        \parbox{1.8cm}{$
    \langle \mathtt{while}\ b\ \mathtt{do}\ \\
    S, s' \rangle \to s''
        $}
    }
  \infer[
...
\begin{itemize}
    ...
    annotations to justify the choice of a rule. Where the justification can be proven outside the derivation tree,
...
\[
    ...
    \end{prooftree}
\]
where \(T_1\) is a derivation tree with root \\
\(\langle S,\ s\rangle \to s'\) and \(T_2\) is a derivation tree
with root \\
\(\langle \mathtt{while}\ b\ \mathtt{do}\ S,\ s'\rangle \to s''\). \\
...
As we can see the Lean version mimics the textbook very closely.
This proof builds the derivation tree from the top and moving down towards the
...
To show what actually is happening a table of translation is provided in the \autoref{table:TLDR} below:
\begin{figure}[H]
    ...
    \begin{tabularx}{\textwidth}{XXX}
        ...
        \(
        \Gamma, (\text{h\_p} : \mathtt{Prop}_1) \vdash \mathtt{Prop}_2
        \)
        &
        Given an implication of the form \(\Gamma \vdash \mathtt{Prop}_1 \implies \mathtt{Prop}_2\),
        it is enough to prove that \(\Gamma' \vdash \mathtt{Prop}_2\) where \(\Gamma' = \Gamma, \mathtt{Prop}_1\).
        &
        Assuming \(\mathtt{Prop}_1\) holds, we can then prove that \(\mathtt{Prop}_2\) by \dots \\ \\

        Given the goal in lean:
        \(\langle \mathtt{Stmt}, s \rangle\) \(\to_{ns} s''\)
        we can solve it using:\\
        \lean{cases Stmt with}\qquad \quad
        \lean{| case_1 .. => ...}\qquad
        \lean{| case_2 .. => ...}\qquad
        \lean{| case_? .. => ...}\qquad
        \lean{| case_k .. => ...}
...
\end{leftcolumn}
\begin{rightcolumn}
The only rule that could be applied to give \\
\(\langle S_1;\ S_2,\ s \rangle \to s''\) is
...
to the premise \\
\(\langle S_2,\ s_0 \rangle \to s'\) and from \(\langle S_2,\ s \rangle \to s''\) we get \(s' = s''\) as required.
\end{rightcolumn}
\vspace{2em}
...
alternative \\
\(\langle \mathtt{if}\ b\ \mathtt{then}\ S_1\ \mathtt{else}\ S_2,\ s \rangle \to s''\) is 
...
because
\[
\mathcal{B}[\![b]\!] = \mathbf{tt}, \ \langle S,\ s \rangle \to s_0, \ \langle \mathtt{while}\ b\ \mathtt{do}\ S,\ s_0 \rangle \to s'
\]
The only rule that could be applied to give \\
\(\langle \mathtt{while}\ b\ \mathtt{do}\ S,\ s \rangle \to s''\)
...
Thus we have
\[
\langle \mathtt{while}\ b\ \mathtt{do}\ S,\ s_0 \rangle \to s', \ \langle \mathtt{while}\ b\ \mathtt{do}\ S,\ s_0 \rangle \to s''
\]
...
handled via manual applications of the underlying \texttt{small\_step\_k} and
\texttt{small\_step\_star} relations.
...
\begin{leftcolumn}
    \begin{Lean_Code}
      | comp1 progress =>
        ...
        specialize ih k§\(_0\)§
          h_lt h_rest
        have §\(\langle\)§s§\(_0\)§, k§\(_1\)§, k§\(_2\)§,
          hk§\(_1\)§, hk§\(_2\)§, h_sum§\(\rangle\)§ := ih
    \end{Lean_Code}
\end{leftcolumn}
\begin{rightcolumn}
...
\begin{itemize}
    ...
    hypothesis is not bound to the specific initial state \(s\) using "\lean{generalizing s}",
    otherwise the proof will fail when the state changes during a step.
...
Hence, \(S\) will have to loop on state \(s\) also in the other semantics~\citep[p.~41 - 42]{nielson_semantics_2007}.
\end{quotation}
...
because
\[
\mathcal{B}[\![b]\!] = \mathbf{tt}, \ \langle S,\ s \rangle \to s', \ \langle \mathtt{while}\ b\ \mathtt{do}\ S,\ s' \rangle \to s''
\]
The induction hypothesis can be applied to both of the premises \\
\(\langle S,\ s \rangle \to s'\)
...
something which Lean does automatically for us, allowing us to write shorter proofs, such in the case of
if true case.
...
\langle S_1,\ s \rangle \to s'\ \mbox{and}\ \langle S_2,\ s' \rangle \to s''
\]
Using [comp$_{\text{ns}}$], we now get required \\
\(\langle S_1;\ S_2,\ s \rangle \to s''\)
\end{rightcolumn}
\vspace{2em}
...
\begin{itemize}
    ...
    Compared to Pen-and-Paper proofs, Lean is very picky in what it can prove about partial functions.
\end{itemize}
